...
def npu_profiler_session_active():
    ...
    try:
        ...
        log.debug("could not probe the torch_npu profiler state: %s", exc)  # noqa: G200
        return False
...
class NPUCachingAutotuner(CachingAutotuner):
    ...
    def _save_precompile_runtime_inputs(self, args, kwargs):
        pass

    def _prepare_precompile(self):
        self._prepare_configs_for_precompile()
    ...
    def _precompile_current_configs(self, compile_fn):
        compile_configs = self._precompile_variant_configs(self.configs)
        self.compile_results = compile_fn(compile_configs)
    ...
    def _precompile_worker_with(self, compile_fn):
        ...
        self._precompile_current_configs(compile_fn)
    ...
    def _precompile_config(self, cfg: Config) -> TritonCompileResultNpu:
        ...
        try:
            ...
            log.debug("config: %s compile failed, cost time: %ss", cfg_kwargs, time.perf_counter() - start_time)
            if binary is None:
            ...
            if match:
                ...
                cfg.real_ub_size = required_ub_bits
            raise
        log.debug("config: %s compile success, cost time: %ss", cfg_kwargs, time.perf_counter() - start_time)
        return TritonCompileResultNpu(binary, cfg, compile_meta, self.inductor_meta)
    ...
    def _benchmark_candidate_entries(self, *args, **kwargs):
        ...
        for entry in self.compiled_candidate_entries:
            ...
            try:
                ...
                log.debug(
                    "PreRun [%s], candidate: %s\n"
                    " variant [%s] runtime_blocks [%s] "
                    "success, elapsed: %.3f ms",
                    self.fn.__name__,
                    candidate_id,
                    launcher.config,
                    runtime_blocks,
                ...
                if autotune_continue_on_failure:
                    ...
                    raise RuntimeError(prerun_log_str) from e
    ...
    def data_dump(self, *args, dump_path=None):
        ...
        input_info = {
            i: list(arg.shape) if isinstance(arg, torch.Tensor) else type(arg).__name__
            for i, arg in enumerate(args)
        }

        if self._dump_counter == 0:
            log.info("[ACC_DEBUG] %s invocation=#1 saving args=%s to data.pth",
                     self.get_fn_name(), input_info)
            torch.save(args, data_dump_path)
            self._dump_counter += 1
        else:
            log.debug("[ACC_DEBUG] %s invocation=#%s skip save (data.pth preserved from invocation #1, args=%s)",
                      self.get_fn_name(), self._dump_counter + 1, input_info)
        return True
    ...
    def autotuner(self, *args, stream, benchmark_run=False, **kwargs):
        ...
        if len(self.launchers) == 0:
            self._save_precompile_runtime_inputs(args, kwargs)
            start_time = time.time_ns()
            self.precompile()
        ...
        if candidate_count == 1:
            ...
            log.info("%s benchmark elapsed time %ss", self.get_fn_name(), time.perf_counter() - autotune_start_time)
            return
...
class NPUCostModelAutotuner(NPUCachingAutotuner):
    def __init__(
            self,
            fn,
            triton_meta,
    ...
    def _apply_costmodel_to_configs(self, *args, **kwargs):
        ...
        try:
            ...
            log.warning("Skip costmodel prefilter because costmodel path is unavailable: %s", exc)  # noqa: G200
            return
        ...
        if ranked_cfgs:
            ...
            filtered_win_configs = ranked_cfgs[:selected_count]
            filtered_out_configs = ranked_cfgs[selected_count:]
            self.configs = filtered_win_configs
            self._costmodel_fallback_configs = filtered_out_configs or None
...
class NPUDebugAutotuner(NPUCostModelAutotuner):
...
def cached_autotune(
    ...
    def decorator(fn):
        ...
        autotuner_cls = (
            NPUCostModelAutotuner
            if npu_config.enable_costmodel_prefilter
            else NPUCachingAutotuner
        )
        return autotuner_cls(
            fn,
            triton_meta=triton_meta,
            inductor_meta=inductor_meta,
            configs=configs,
...
def _triton_config_npu_index_grouped(
    ...
    log.info(
        "grouped codegen groups: kernel=%s group_id_count=%s "
        "reachable_group_count=%s total_candidate_count=%s candidate_count_by_group=%s",

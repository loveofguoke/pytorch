...
operator_to_string = MLIR_OPERATOR_MAPPING.operator_to_string
string_to_operator = MLIR_OPERATOR_MAPPING.string_to_operator

def map_operators_to_strings(expr_str: str):
    return _map_operators_to_strings(expr_str, MLIR_OPERATOR_MAPPING)
...
def create_sym_inputs(traced_graph: TracedGraph, size: List[Expr]):
    return _create_sym_inputs(traced_graph, size, MLIR_OPERATOR_MAPPING)

def process_ir_constant(inp: ExpandView) -> Union[TracedGraph, int, float]:
    return _process_ir_constant(inp, MLIR_OPERATOR_MAPPING)

def fetch_graphs(inputs: Optional[List[TensorBox]]):
    return _fetch_graphs(inputs, MLIR_OPERATOR_MAPPING, use_npu_meta=False)

def merge_traced_graphs(input_graphs: List[TracedGraph], origin_fn, node_name, **kwargs):
    return _merge_traced_graphs(
        input_graphs, origin_fn, node_name, MLIR_OPERATOR_MAPPING, **kwargs
    )
...
def make_pointwise(
    ...
    **kwargs
    ...
    def inner(*inputs: TensorBox, alpha=None):
        ...
        is_gpu_device = ir.is_gpu(decode_device(inputs[0].get_device()).type)
        ...
        if not override_device:
            device = None
            for i in inputs:
                if ir.is_gpu(i.get_device().type):
                    device = i.get_device()
                    break
            if not device:
...
@register_lowering(torch.ops.npu._npu_dtype_cast, type_promotion_kind=None)
...
def to_device(x: TensorBox, device: torch.device, *, copy=False, non_blocking=False):
    ...
    return TensorBox.create(ir.DeviceCopy.create(x, device, non_blocking, traced_graph=new_graph, node_name=node_name))
...
def unfold(x, dimension, size, step):
    ...
    if sizevars.guarding_hint_or_throw(dim_size) > 0:
        x.mark_reuse(sizevars.guarding_hint_or_throw(CeilDiv(new_dim_size * size, dim_size)))
...
def make_fallback(op, layout_constraint=None, warn=True, override_decomp=False, get_decomp_fn=None):
...
def _full(fill_value, device, dtype, size):
    value = fill_value
    dtype = torch.int32 if dtype == torch.int64 else dtype
...
def should_not_sum(a, b, keepdims):
    if not b:
        return False
    for i in b:
        if i < 0 or i >= len(a):
...
def sum_(x, axis=None, keepdims=False, *, dtype=None):
    ...
    if anir_config.disable_any_pbr and axis and len(x.get_size()) > 1 and axis[0] < len(x.get_size()) - len(axis):
        fn = fallback_handler(
            aten.sum.dim_IntList, add_to_fallback_set=False
        )
    else:
...
def neg(a):
    if a.get_dtype() in (torch.int32, torch.int64):
        return mul(a, -1)
    fn = ops_wrapper(aten.neg.__name__)
    fn = register_fn_to_aten_fn(fn, aten.neg)
...
register_lowering(aten.clamp_min)(clamp_min)
register_lowering(aten.clamp_max)(clamp_max)
abs = register_pointwise(aten.abs)
...
from torch._inductor.codegen.common import BackendFeature, pointwise_overrides_data
...
from torch._inductor.comm_lowering import register_comm_lowerings
...
def prepare_softmax_online(x, dim):
    ...
    if num_split == 1 and V.graph.sizevars.statically_known_geq(rnumel, config.unroll_reductions_threshold):

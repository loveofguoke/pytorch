...
class TorchCompileTriggerTests(unittest.TestCase):
    def run_in_subprocess(self, code, timeout=60):
        env = os.environ.copy()
    ...
    # Verify NPUGraphs rejects options kwargs for every registration order.
    def test_npugraphs_rejects_options_across_registration_order(self):
        self.run_in_subprocess(
            ...
            with mock.patch.object(
                ...
                # The public entry point accepts Dynamo's generic kwargs but
                # deliberately does not pass them to the baseline backend.
                assert _npugraphs_backend_entrypoint(
                    gm, inputs, options=options
                ) == "unexpected"

                # The registered backend itself retains its no-options API.
                try:
    ...
    def test_npu_export_public_entry_and_import_order_matrix(self):
        ...
        if hasattr(torch.export, "export_for_training"):
            cases["prebound_export_for_training"] = (
    ...
    def test_invalid_inductor_arguments_fail_without_npu_initialization(self):
        cases = {
            ...
            "invalid_hyphenated_npu_option_type": (
                "torch.compile(lambda x: x + 1, "
                'options={"npu-backend": 1})',
                "Unexpected type of attr npu-backend",
            ),
        }
    ...
    # Verify a real torch.compile Inductor entry initializes the full stack.
    # Patch only the final compile_fx lowering step so this test exercises the
    # NPU Dynamo/Inductor wrapper and lazy setup without compiling a Triton
    # kernel in the environment.
    ...
    def test_npu_inductor_initialization_chain(self):
        self.run_in_subprocess(
            ...
            def fake_compile_fx(model, inputs, **kwargs):
                compile_calls.append((model, inputs, kwargs))
                return lambda *args, **kwargs: args[0]

            with mock.patch.object(compile_fx, "compile_fx", fake_compile_fx):
                compiled(x)
